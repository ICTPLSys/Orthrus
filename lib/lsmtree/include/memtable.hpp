...
struct SkipListNode {
    ...
    SkipListNode(KeyT key, ValueT value, int lvl,
                 const SkipListNode* nxt = nullptr);
...
public:
    ...
    const SkipListNode* findCache(const SkipListNode* p, const Data* p_data,
                                  int level, KeyT key) const;
