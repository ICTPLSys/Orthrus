...
SkipListNode::SkipListNode(KeyT key, ValueT value, int lvl,
                           const SkipListNode* nxt)
    : key(key), level(lvl) {
    data = ptr_t<Data>::create({
        key,
        value,
        false,
    });
...
SkipList::SkipList() {
    ...
    // level_ = new int(0);
    tail_ = new SkipListNode{SKIPLIST_KEY_INVALID, 0, 0,
                             static_cast<SkipListNode*>(nullptr)};
    head_ =
        new SkipListNode{SKIPLIST_KEY_INVALID, 0, SKIPLIST_MAX_LEVEL, tail_};
...
const SkipListNode* SkipList::findCache(const SkipListNode* p,
                                        const Data* p_data, int level,
                                        KeyT key) const {
#if (LSMTREE_ENABLE_SKIPLIST_CACHE_ACCESS)
