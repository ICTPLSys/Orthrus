...
int main_fn(const char *buffer, size_t file_size, map_reduce_config config) {
#ifdef PROFILE_MEM
    profile::mem::init_mem("phoenix-memory_status-rbv-replica.log");
    profile::mem::start();
#endif
...

    {
        ...
        assert(len > 0);
        long long client_us =
            atoll(std::string(clock_reader.packet, len - 1).c_str());
    ...
    for (size_t k = 0; k < config.n_mappers; ++k) {
        ...
        mapper_threads.emplace_back([&, client_fd] {
            ...
            while (active_threads.load() >= config.n_threads * 4)
                my_usleep(rand() % 10000);
            ...
            profile::record_validation_cpu_time(
                (end_us - start_my_us) * kCpuMhzNorm, 1);
#endif
        });
    }
    for (size_t i = 0; i < config.n_mappers; ++i) {
    ...
    for (size_t k = 0; k < config.n_reducers; ++k) {
        ...
        reducer_threads.emplace_back([&, client_fd] {
            ...
            std::string s = std::string(reader.packet + 20, len - 21);
            rbv::hasher.deserialize(s);
            raw::word_count_reduce_worker(map_results, reduce_results, i,
                                          config);
            rbv::hasher.finalize();
            long long end_us = profile::get_us_abs();
            active_threads.fetch_sub(1);
    ...
    auto end_time = rdtsc();
    std::cout << std::setw(16) << "Time(Replica) taken: "
              << microsecond(start_time, end_time) / 1000 << " ms" << std::endl;
