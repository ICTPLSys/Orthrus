...
std::pair<map_reduce_config, size_t> parse_args(int argc, char **argv) {
    ...
    sprintf(config.replica_ip, "%s",
            vm["replica-ip"].as<std::string>().c_str());
...
int main_fn(const char *buffer, size_t file_size, map_reduce_config config) {
#ifdef PROFILE_MEM
    profile::mem::init_mem("phoenix-memory_status-rbv-primary.log");
    profile::mem::start();
#endif
    ...
    {
        int clock_fd =
            connect_server(config.replica_ip, config.replica_port + 1);
