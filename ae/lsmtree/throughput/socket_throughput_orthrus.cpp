        ...
        while (auto len = read_packet(sizeof(Msg))) {
            ...
            case Cmd::Set: {
                ...
                // printf("%d\n", msg.key);
                {
                    ...
                    scee::run2<int>(app::lsmtree_set, validator::lsmtree_set,
        ...
        while (isRunning.load()) {
            ...
            for (int i = 0; i < nfds; ++i) {
                if (events[i].data.fd == fd) {
                    ...
                    int client_fd =
                        accept(fd, (sockaddr*)&client_address, &client_len);
...
int main(int argc, char* argv[]) {
    ...
    profile::mem::init_mem("lsmtree-memory_status-orthrus.log");

    gflags::SetUsageMessage("Usage: ./socket [--total-ops=<int>]");
    gflags::ParseCommandLineFlags(&argc, &argv, true);
    auto drop = ScopeGuard([&]() { gflags::ShutDownCommandLineFlags(); });

#if (ENABLE_PROFILE_VAL_CDF)
    profile::start("lsmtree-validation_latency-orthrus.cdf");
    // scee::main_thread([]() {
    Server server;
    server.Run();
    //     return 0;
