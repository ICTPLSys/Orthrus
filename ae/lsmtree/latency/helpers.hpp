...
static ssize_t read_all(int fd, void* buf, ssize_t len) {
    ...
    while (bytes_read < len) {
        ssize_t ret =
            read(fd, (void*)((char*)buf + bytes_read), len - bytes_read);
        if (ret < 0) {
...
void print_stats(hdr_histogram* validation_latency_histogram,
                 std::string filename) {
    std::cout << "----------------------------------------" << std::endl;
